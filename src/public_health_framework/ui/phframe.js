...
class PHConnectorConsole extends PHElement {
  ...
  async providerChanged() {
    ...
    const presets = {
      ...
      dhis2: ["DHIS2 OAuth connection", "Sign in on your DHIS2 server, approve PHFrame, and return automatically. PHFrame encrypts the OAuth credential and discovers available data sets.", "BfMAe6Itzgt", "Data Set UID", false],
    ...
    guide.innerHTML = `<b>${presets[0]}</b><span>${presets[1]}</span>${type === "dhis2" ? (connected ? `<span class="ph-oauth-connected"><i></i> Connected as ${PHFrame.escape(this.dhis2Status.user?.displayName || this.dhis2Status.user?.username || "DHIS2 user")}</span><div class="ph-dhis2-import"><div class="ph-field"><label>DHIS2 data set</label><select data-dhis2-data-set><option value="">Loading authorized data sets…</option></select></div><div class="ph-field"><label>New PHFrame dataset name</label><input data-dhis2-local-name pattern="[a-z][a-z0-9_]*" placeholder="malaria_monthly_data"></div><div class="ph-field"><label>Automatic sync (minutes)</label><input data-dhis2-schedule type="number" min="1" value="60"></div><button type="button" class="ph-button" data-dhis2-import>Create dataset and connector</button></div><button type="button" class="ph-button ph-button-secondary" data-dhis2-disconnect>Disconnect</button>` : (this.dhis2Status?.available ? `<button type="button" class="ph-button" data-dhis2-connect>Connect with DHIS2</button>` : `<div class="ph-oauth-setup"><b>One-time administrator setup required</b><span>Register PHFrame as an OAuth client in this DHIS2 instance, using <code>${PHFrame.escape(location.origin)}/api/integrations/dhis2/callback</code>, then configure the client ID and secret on the PHFrame server.</span></div>`)) : ""}`;
    ...
    const form = this.querySelector("[data-connector-form]"), regularFields = [...form.querySelectorAll(".ph-form-grid > .ph-field")], mappingField = form.querySelector("textarea[name=mapping]").closest(".ph-field"), createButton = form.querySelector(':scope > button[type="submit"]');
    ...
    guide.querySelector("[data-dhis2-connect]")?.addEventListener("click", () => { if (!server.reportValidity()) return; location.href = `/api/integrations/dhis2/connect?server_url=${encodeURIComponent(server.value)}`; });
