...
class PHAIWorkspace extends PHElement {
  ...
  async review(id, decision) {
    ...
    if (!note?.trim()) return;
    try { await PHFrame.send(`/api/ai/summaries/${id}/review`, "POST", { decision, note }); PHFrame.notify(`Draft ${decision}.`); this.render(); } catch (error) { PHFrame.notify(error.message); }
  }
}
