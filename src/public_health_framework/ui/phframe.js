...
class PHMap extends PHElement {
  async render() {
    ...
    try {
      ...
      if (layers.data.length) {
        ...
        this.renderBoundary(boundary.data, values);
        return;
      }
  ...
  renderBoundary(boundary, values) {
    const features = boundary.geojson.features, coordinates = features.flatMap(feature => this.geometryPoints(feature.geometry));
    if (!coordinates.length) { this.innerHTML = `<p class="ph-error">Installed boundary has no polygon geometry.</p>`; return; }
    const xs = coordinates.map(point => point[0]), ys = coordinates.map(point => point[1]), bounds = [Math.min(...xs), Math.min(...ys), Math.max(...xs), Math.max(...ys)];
    const counts = new Map(values.map(item => [this.normalizePlace(item.value), item.count])), maximum = Math.max(1, ...values.map(item => item.count));
    const shapes = features.map(feature => { const properties = feature.properties || {}, name = properties.shapeName || properties.name || properties.NAME_1 || properties.NAME_2 || properties.NAME || "Area", count = counts.get(this.normalizePlace(name)) || 0, opacity = count ? .2 + count / maximum * .8 : .08; return `<path d="${this.geometryPath(feature.geometry, bounds)}" fill="var(--ph-color-primary)" fill-opacity="${opacity}"><title>${PHFrame.escape(name)}: ${count}</title></path>`; }).join("");
    this.innerHTML = `<div class="ph-widget-body ph-boundary-map"><svg class="ph-chart" viewBox="0 0 720 420" role="img" aria-label="${PHFrame.escape(boundary.country)} administrative boundary choropleth">${shapes}</svg><p class="ph-muted">${PHFrame.escape(boundary.country)} ${PHFrame.escape(boundary.level)} · hover areas for values · geoBoundaries, CC BY 4.0</p></div>`;
  }
...
class PHDashboardManager extends PHElement {
  ...
  publishDialog(active) {
    if (!active) return "";
    const project = String(PHFrame.siteSettings.cloudflare_project_name || active.title || "dashboard").toLowerCase().replace(/[^a-z0-9-]+/g, "-").replace(/^-|-$/g, "").slice(0, 58);
    return `<dialog class="ph-dialog ph-publish-dialog"><form method="dialog" class="ph-stack" data-publish-form><div class="ph-widget-header"><div><p class="ph-eyebrow">Privacy-aware publication</p><h2>Publish this dashboard</h2></div><button value="cancel" class="ph-dialog-close" aria-label="Close">×</button></div><p class="ph-muted">Only aggregate visualization output is exported. PHFrame blocks protected fields before creating a bundle or deployment.</p><div class="ph-form-grid"><div class="ph-field"><label>Publication mode</label><select name="mode"><option value="snapshot">Snapshot — fixed data</option><option value="live">Live — refresh from aggregate API</option></select></div><div class="ph-field"><label>Cloudflare project name</label><input name="project_name" value="${PHFrame.escape(project)}" pattern="[a-z0-9][a-z0-9-]*[a-z0-9]|[a-z0-9]" required></div></div><div class="ph-stack" data-live-fields hidden><div class="ph-field"><label>HTTPS aggregate API URL</label><input name="upstream_url" type="url" placeholder="https://api.example.org/dashboard-feed"></div><div class="ph-field"><label>Refresh/cache interval (minutes)</label><input name="refresh_minutes" type="number" min="1" max="1440" value="15"></div><p class="ph-muted">If the API needs a bearer token, add the <code>UPSTREAM_API_TOKEN</code> secret to the deployed Cloudflare Pages project.</p></div><div class="ph-card ph-publication-review" data-publication-review><b>Privacy review required</b><span>Publish or download to run the aggregate-only review.</span></div><div class="ph-actions"><button class="ph-button" value="publish">Publish to Cloudflare</button><button class="ph-button ph-button-secondary" type="button" data-download-bundle>Download bundle</button><button value="cancel">Cancel</button></div><p class="ph-status" role="status" data-publication-status></p></form></dialog>`;
  }
  ...
  async publishDashboard(event, active) { if (event.submitter?.value !== "publish") return; event.preventDefault(); const status = this.querySelector("[data-publication-status]"), button = event.submitter; button.disabled = true; status.textContent = "Reviewing privacy and deploying…"; try { await this.reviewPublication(active); const response = await PHFrame.send("/api/publications/deploy", "POST", this.publicationPayload(active)); status.innerHTML = `Published: <a href="${PHFrame.escape(response.data.url)}" target="_blank" rel="noopener">${PHFrame.escape(response.data.url)}</a>`; PHFrame.notify("Dashboard published to Cloudflare."); } catch (error) { status.textContent = error.message; status.className = "ph-status ph-error"; } finally { button.disabled = false; } }
...
class PHSettingsPanel extends PHElement {
  ...
  render() {
    ...
    this.querySelector(".ph-settings-save").insertAdjacentHTML("beforebegin", `<section class="ph-card ph-stack ph-boundary-settings"><div><p class="ph-eyebrow">Geographic boundaries</p><h3>Built-in country boundary downloads</h3></div><p class="ph-muted">Download open administrative boundaries for real choropleth maps. Use ISO3 codes such as BGD, USA, GBR, or KEN.</p><div class="ph-form-grid"><div class="ph-field"><label>Country ISO3</label><input data-boundary-country maxlength="3" pattern="[A-Za-z]{3}" placeholder="BGD"></div><div class="ph-field"><label>Administrative level</label><select data-boundary-level><option>ADM0</option><option>ADM1</option><option selected>ADM2</option><option>ADM3</option><option>ADM4</option><option>ADM5</option></select></div></div><button class="ph-button" type="button" data-boundary-download>Download boundary layer</button><p class="ph-status" role="status" data-boundary-status></p><div data-boundary-list></div><small class="ph-muted">Boundary data: geoBoundaries gbOpen, CC BY 4.0. Attribution is required.</small></section>`);
    this.querySelector(".ph-settings-save").insertAdjacentHTML("beforebegin", `<section class="ph-card ph-stack ph-cloudflare-settings"><div><p class="ph-eyebrow">Public dashboard hosting</p><h3>Cloudflare Pages</h3></div><p class="ph-muted">Publish aggregate-only dashboard snapshots or live dashboards to a free <code>pages.dev</code> address. PHFrame never saves the API token.</p><div class="ph-form-grid"><div class="ph-field"><label>Cloudflare account ID</label><input name="cloudflare_account_id" value="${PHFrame.escape(this._settings.cloudflare_account_id || "")}" autocomplete="off" placeholder="32-character account ID"></div><div class="ph-field"><label>Default Pages project name</label><input name="cloudflare_project_name" value="${PHFrame.escape(this._settings.cloudflare_project_name || "")}" pattern="[a-z0-9][a-z0-9-]*[a-z0-9]|[a-z0-9]" placeholder="my-public-health-dashboard"></div></div><div class="ph-field"><label>API token environment variable</label><input name="cloudflare_token_env" value="${PHFrame.escape(this._settings.cloudflare_token_env || "CLOUDFLARE_API_TOKEN")}" pattern="[A-Z_][A-Z0-9_]*"></div><p class="ph-provider-guide"><b>Keep the token outside PHFrame.</b><span>Before starting the app, set <code>export ${PHFrame.escape(this._settings.cloudflare_token_env || "CLOUDFLARE_API_TOKEN")}=your_token</code>. The token needs Cloudflare Pages edit permission.</span></p><div data-publication-history><p class="ph-muted">Loading publication history…</p></div></section>`);
    ...
    this.loadBoundaries(); this.loadBoundaryCountries(); this.loadPublications();
  }
