...
class PHDashboardManager extends PHElement {
  ...
  createDialog() {
    const count = Object.keys(this._metadata.datasets || {}).length;
    return `<dialog class="ph-dialog ph-template-dialog"><form method="dialog" class="ph-stack"><div class="ph-widget-header"><div><p class="ph-eyebrow">Dashboard templates</p><h2>Create a professional dashboard</h2></div><button value="cancel" class="ph-dialog-close">×</button></div><div class="ph-field"><label>Dashboard name</label><input name="title" required placeholder="National programme overview"><small>Dashboards are independent workspaces. You can use visualizations from any of the ${count} installed datasets.</small></div><div class="ph-template-grid"><button type="button" data-template="overview"><b>Executive overview</b><span>Best headline metrics and trends across available data</span></button><button type="button" data-template="surveillance"><b>Surveillance operations</b><span>Indicators, alerts, locations, and time</span></button><button type="button" data-template="programme"><b>Programme monitoring</b><span>Coverage-style metrics and disaggregation</span></button><button type="button" data-template="dhis2"><b>DHIS2 aggregate</b><span>Data values, elements, and category combinations</span></button><button type="button" data-template="geospatial"><b>Worldwide geospatial</b><span>Coordinate maps from every compatible dataset</span></button><button type="button" data-template="blank"><b>Blank canvas</b><span>Add content from any installed dataset</span></button></div><p class="ph-muted" data-template-advice></p></form></dialog>`;
  }
  customizeDialog() {
  ...
  updateTemplateRecommendations() {
    const capabilities = Object.keys(this._metadata.datasets || {}).map(dataset => ({ dataset, ...this.capabilities(dataset) }));
    this.querySelectorAll("[data-template]").forEach(button => button.classList.remove("ph-template-recommended"));
    const recommended = capabilities.some(item => item.dhis2) ? "dhis2" : (capabilities.some(item => item.latitude && item.longitude) ? "geospatial" : (capabilities.some(item => item.dates.length) ? "surveillance" : "overview"));
    this.querySelector(`[data-template="${recommended}"]`).classList.add("ph-template-recommended");
    const totals = capabilities.reduce((result, item) => ({ numeric: result.numeric + item.numeric.length, categories: result.categories + item.categories.length, dates: result.dates + item.dates.length }), { numeric: 0, categories: 0, dates: 0 });
    this.querySelector("[data-template-advice]").textContent = `Recommended: ${this.querySelector(`[data-template="${recommended}"] b`).textContent}. PHFrame examined ${capabilities.length} datasets with ${totals.numeric} numeric, ${totals.categories} categorical, and ${totals.dates} date columns.`;
  }
  templateWidgets(template) {
    const sources = Object.entries(this._metadata.datasets || {}).map(([dataset, item]) => ({ dataset, label: item.label, ...this.capabilities(dataset) }));
    const usable = sources.filter(item => item.numeric.length || item.categories.length || item.dates.length), widgets = [{ _id: `content-${Date.now()}`, type: "content", title: "About this dashboard", html: `<p><strong>Multi-dataset dashboard</strong></p><p>This workspace can combine compatible visualizations from all installed datasets. Edit this text to explain the purpose, reporting period, and audience.</p>` }];
    usable.filter(item => item.numeric[0]).slice(0, 3).forEach((item, index) => widgets.push({ _id: `metric-${index}-${Date.now()}`, type: "field_kpi", title: `${item.label} · Total ${item.numeric[0].replaceAll("_", " ")}`, dataset: item.dataset, field: item.numeric[0], operation: "sum" }));
    const category = usable.find(item => item.categories[0]); if (category) widgets.push({ _id: `groups-${Date.now()}`, type: "field_chart", title: `${category.label} · ${category.categories[0].replaceAll("_", " ")} distribution`, dataset: category.dataset, field: category.categories[0] });
    const trend = usable.find(item => item.dates[0] && item.numeric[0]); if (trend) widgets.push({ _id: `trend-${Date.now()}`, type: "epi_curve", title: `${trend.label} · ${trend.numeric[0].replaceAll("_", " ")} over time`, dataset: trend.dataset, date_field: trend.dates[0], value_field: trend.numeric[0] });
    if (template === "dhis2") usable.filter(item => item.dhis2).slice(0, 2).forEach((item, index) => item.categories[0] && widgets.push({ _id: `dhis2-${index}-${Date.now()}`, type: "field_chart", title: `${item.label} · ${item.categories[0].replaceAll("_", " ")} breakdown`, dataset: item.dataset, field: item.categories[0] }));
    if (template === "surveillance") Object.entries(this._metadata.indicators || {}).slice(0, 3).forEach(([indicator, item]) => widgets.push({ _id: `indicator-${indicator}-${Date.now()}`, type: "kpi", title: item.label, indicator, dataset: item.dataset }));
    if (template === "geospatial") usable.filter(item => item.latitude && item.longitude).slice(0, 2).forEach((item, index) => widgets.push({ _id: `map-${index}-${Date.now()}`, type: "geo_map", title: `${item.label} · Geographic distribution`, dataset: item.dataset, latitude_field: item.latitude, longitude_field: item.longitude }));
    return template === "blank" ? [] : widgets;
  }
  async createDashboard(template) {
    const form = this.querySelector(".ph-template-dialog form"), title = form.elements.title.value.trim();
    if (!title) { form.elements.title.reportValidity(); return; }
    const widgets = this.templateWidgets(template), datasets = [...new Set(widgets.map(item => item.dataset).filter(Boolean))];
    const dashboard = { id: `dashboard-${Date.now()}`, title, description: `${template.replaceAll("_", " ")} template · ${datasets.length} data source${datasets.length === 1 ? "" : "s"}`, datasets, template, widgets };
  ...
  async customizeDashboard(event, dashboard) { if (event.submitter?.value === "cancel") return; event.preventDefault(); const title = new FormData(event.currentTarget).get("title").trim(); if (!title) return; const widgets = dashboard.widgets.map((widget, index) => ({ ...widget, _id: widget._id || `widget-${index}-${Date.now()}` })), datasets = [...new Set(widgets.map(item => item.dataset || this._metadata.indicators?.[item.indicator]?.dataset || this._metadata.dimensions?.[item.dimension]?.dataset).filter(Boolean))]; const copy = { id: `dashboard-${Date.now()}`, title, description: dashboard.description || "Customized project dashboard", datasets, template: "custom", widgets }; await this.persist([...(PHFrame.siteSettings.dashboards || []), copy]); localStorage.setItem("ph-active-dashboard", copy.id); this.querySelector(".ph-customize-dialog").close(); this.render(); }
...
class PHDashboard extends PHElement {
  ...
  widgetDialog() {
    const datasetLabel = dataset => this.metadata.datasets?.[dataset]?.label || dataset || "Configured source";
    const indicators = Object.entries(this.metadata.indicators || {}).map(([name, item]) => `<option value="kpi|${name}">Metric · ${PHFrame.escape(datasetLabel(item.dataset))} · ${PHFrame.escape(item.label)}</option>`).join("");
    const dimensions = Object.entries(this.metadata.dimensions || {}).map(([name, item]) => `<option value="chart|${name}">Category · ${PHFrame.escape(datasetLabel(item.dataset))} · ${PHFrame.escape(item.label)}</option>`).join("");
  ...
  addWidget(form) {
    ...
    if (widget.type === "kpi") { widget.indicator = parts[1]; widget.dataset = this.metadata.indicators?.[parts[1]]?.dataset; }
    else if (widget.type === "field_kpi") { widget.dataset = parts[1]; widget.field = parts[2]; widget.operation = "sum"; }
    else if (widget.type === "chart") { widget.dimension = parts[1]; widget.dataset = this.metadata.dimensions?.[parts[1]]?.dataset; }
...
class PHImportWizard extends PHElement {
  ...
  async preview() {
    ...
    try {
      ...
      const profileRows = profile.columns.map(column => `<tr><td><b>${PHFrame.escape(column.name)}</b></td><td><span class="ph-profile-type">${PHFrame.escape(column.semantic_type)}</span><small>${PHFrame.escape(column.storage_type)}</small></td><td>${Math.round(column.confidence * 100)}%</td><td>${column.missing_percent}%</td><td>${column.distinct}</td><td>${column.examples.map(PHFrame.escape).join(", ") || "—"}</td></tr>`).join("");
      const issueRows = quality.issues.slice(0, 20).map(issue => `<tr><td><span class="ph-quality-severity ph-${issue.severity}">${issue.severity}</span></td><td>${PHFrame.escape(issue.rule)}</td><td>${PHFrame.escape(issue.field || "All rows")}</td><td>${issue.affected_count}</td><td>${PHFrame.escape(issue.message)}</td></tr>`).join("") || `<tr><td colspan="5">No quality issues detected.</td></tr>`;
      this.querySelector("[data-workspace]").innerHTML = `<div class="ph-import-step ph-profile-review"><div class="ph-step-heading"><span>02</span><div><h3>Data health and profile</h3><p>PHFrame staged an immutable source version, inferred meaning, and ran deterministic quality checks.</p></div></div><div class="ph-profile-summary"><div><b>${profile.row_count}</b><small>Rows</small></div><div><b>${profile.column_count}</b><small>Columns</small></div><div><b>${quality.score}</b><small>Quality score</small></div><div><b>${quality.blocker_count}</b><small>Blocking issues</small></div></div><div class="ph-quality-banner ph-quality-${quality.readiness}"><b>${PHFrame.escape(quality.readiness.replaceAll("_", " "))}</b><span>${quality.issue_count} issue groups · staged version #${response.data.version.id}</span></div><div class="ph-table-wrap"><table class="ph-table"><thead><tr><th>Severity</th><th>Rule</th><th>Field</th><th>Affected</th><th>Explanation</th></tr></thead><tbody>${issueRows}</tbody></table></div><details><summary>Column intelligence</summary><div class="ph-table-wrap"><table class="ph-table"><thead><tr><th>Column</th><th>Suggested role</th><th>Confidence</th><th>Missing</th><th>Distinct</th><th>Examples</th></tr></thead><tbody>${profileRows}</tbody></table></div></details></div><div class="ph-import-step"><h3>3. Map columns</h3><div class="ph-grid">${mappings}</div><div class="ph-actions"><div class="ph-field"><label for="ph-mapping-name">Save mapping as</label><input id="ph-mapping-name" pattern="[a-z][a-z0-9_]*" placeholder="monthly_cases"></div><button type="button" data-save-mapping>Save mapping</button></div></div><div class="ph-import-step"><h3>4. Review ${response.data.total_rows} rows</h3><div class="ph-table-wrap"><table class="ph-table"><thead><tr>${headers}</tr></thead><tbody>${rows}</tbody></table></div><div class="ph-actions"><button class="ph-button" type="button" data-validate>Validate</button><button class="ph-button" type="button" data-import>Approve and import</button></div></div><div data-results></div>`;
  ...
  async execute(dryRun) {
    ...
    const query = new URLSearchParams({ mapping: JSON.stringify(mapping), dry_run: String(dryRun), version_id: String(this.previewData?.version?.id || "") });
