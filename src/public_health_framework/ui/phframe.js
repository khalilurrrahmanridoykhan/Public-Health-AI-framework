...
class PHImportWizard extends PHElement {
  ...
  async preview() {
    ...
    try {
      ...
      this.querySelector("[data-workspace]").innerHTML = `<div class="ph-import-step ph-profile-review"><div class="ph-step-heading"><span>02</span><div><h3>Data profile</h3><p>PHFrame staged an immutable source version and inferred analytical meaning.</p></div></div><div class="ph-profile-summary"><div><b>${profile.row_count}</b><small>Rows</small></div><div><b>${profile.column_count}</b><small>Columns</small></div><div><b>${profile.duplicate_rows}</b><small>Possible duplicate rows</small></div><div><b>#${response.data.version.id}</b><small>Staged version</small></div></div><div class="ph-table-wrap"><table class="ph-table"><thead><tr><th>Column</th><th>Suggested role</th><th>Confidence</th><th>Missing</th><th>Distinct</th><th>Examples</th></tr></thead><tbody>${profileRows}</tbody></table></div></div><div class="ph-import-step"><h3>3. Map columns</h3><div class="ph-grid">${mappings}</div><div class="ph-actions"><div class="ph-field"><label for="ph-mapping-name">Save mapping as</label><input id="ph-mapping-name" pattern="[a-z][a-z0-9_]*" placeholder="monthly_cases"></div><button type="button" data-save-mapping>Save mapping</button></div></div><div class="ph-import-step"><h3>4. Review ${response.data.total_rows} rows</h3><div class="ph-table-wrap"><table class="ph-table"><thead><tr>${headers}</tr></thead><tbody>${rows}</tbody></table></div><div class="ph-actions"><button class="ph-button" type="button" data-validate>Validate</button><button class="ph-button" type="button" data-import>Approve and import</button></div></div><div data-results></div>`;
