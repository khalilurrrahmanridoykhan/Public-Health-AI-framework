...
class PHSettingsPanel extends PHElement {
  ...
  async downloadBoundary() { const status = this.querySelector("[data-boundary-status]"), button = this.querySelector("[data-boundary-download]"), iso3 = this.querySelector("[data-boundary-country]").value.toUpperCase(), level = this.querySelector("[data-boundary-level]").value; if (!/^[A-Z]{3}$/.test(iso3)) { status.textContent = "Enter a three-letter ISO country code."; return; } button.disabled = true; status.textContent = `Downloading ${iso3} ${level}…`; try { const response = await PHFrame.send("/api/boundaries", "POST", { iso3, level }); status.textContent = `${response.data.country} ${level} installed with ${response.data.feature_count} areas.`; await this.loadBoundaries(); PHFrame.notify("Boundary layer installed."); } catch (error) { status.textContent = error.message; status.className = "ph-status ph-error"; } finally { button.disabled = false; } }
